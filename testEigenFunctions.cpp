...
TEST(EigenTest, JointTrajectory) {
    ...
    auto const &jointValues = jointTrajectory.getJointValues();
    for (auto const &jointValue: jointValues) {
...
TEST(EigenTest, EigenSlidingWindow) {
    ...
    for (int i = 0; i < 20; i++) {
        swEigen.addData(
                Eigen::Vector3d(static_cast<double>(i), static_cast<double>(i * i), static_cast<double>(i * i * i)));
